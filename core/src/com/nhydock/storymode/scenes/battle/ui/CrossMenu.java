...
import com.nhydock.gdx.scenes.scene2d.runnables.SendMessage;
...
import com.nhydock.storymode.util.ListPointer;
...
public class CrossMenu extends Group
{
		...
		addListener(new InputListener(){
		    ...
		    public boolean keyDown(InputEvent evt, int keycode) {
		        ...
		        if (Input.UP.match(keycode)) {
		            MessageManager.getInstance().dispatchMessage(null, sm, Messages.Select, set.peekPrev());
		            return true;
		        }
		        if (Input.DOWN.match(keycode)) {
		            MessageManager.getInstance().dispatchMessage(null, sm, Messages.Select, set.peekNext());
		            return true;
		        }
		        if (Input.ACCEPT.match(keycode)) {
		            MessageManager.getInstance().dispatchMessage(null, sm, Messages.Next, set.get());
		            return true;
		        }
		        if (Input.CANCEL.match(keycode)) {
		            MessageManager.getInstance().dispatchMessage(null, sm, Messages.Prev);
                    return true;
                }
		        return false;
		    }
    ...
    private class ButtonInput extends InputListener {
        ...
        public boolean touchDown(InputEvent evt, float x, float y, int pointer, int button) {
            MessageManager.getInstance().dispatchMessage(0f, null, menu.sm, Messages.Next, this.button);
            return true;
        }
        
        @Override
        public void enter(InputEvent event, float x, float y, int pointer, Actor fromActor) {
            MessageManager.getInstance().dispatchMessage(0f, null, menu.sm, Messages.Select, this.button);
        }
    }
}
