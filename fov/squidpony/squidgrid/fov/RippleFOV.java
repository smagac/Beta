...
public class RippleFOV implements FOVSolver {
     ...
     * @return
     */
    private float getNearLight(int x, int y, float[][] lightMap) {
    ...
    @Override
    public float[][] calculateFOV(float[][] map, int startx, int starty, float force, float decay, RadiusStrategy rStrat, float[][] lightMap) {
        this.map = map;
        this.decay = decay;
        this.startx = startx;
        this.starty = starty;
        ...
        lightSurroundings(startx, starty, lightMap);

        return lightMap;
    }

    private void lightSurroundings(int x, int y, float[][] lightMap) {
        ...
        for (int dx = x - 1; dx <= x + 1; dx++) {
            for (int dy = y - 1; dy <= y + 1; dy++) {
                ...
                if (r <= radius) {
                    float surroundingLight = getNearLight(dx, dy, lightMap);
                    if (lightMap[dx][dy] < surroundingLight) {
                        lightMap[dx][dy] = surroundingLight;
                        lightSurroundings(dx, dy, lightMap);//redo neighbors since this one's light changed
                    }
                }
            }
        }
    ...
    @Override
    public float[][] calculateFOV(float[][] map, int startx, int starty, float radius, float[][] lightMap) {
        return calculateFOV(map, startx, starty, 1, 1 / radius, BasicRadiusStrategy.CIRCLE, lightMap);
    }
}
