...
public class Health {
        ...
        for (Ailment a : status.keys()) {
            if (!a.requiresMedicine()) {
                ...
                if (a.recovered(age)) {
                    status.remove(a, 0);
                    changed = true;
                    MessageDispatcher.getInstance().dispatchMessage(null, Messages.Player.RemoveAilment, a);
                }
            }
        }
        if (changed) {
    ...
    public void reset(){
       status.clear();
       ailments = new ImmutableArray<Ailment>(status.keys().toArray());
       MessageDispatcher.getInstance().dispatchMessage(null, Messages.Player.RemoveAilment, null);
    }
}
