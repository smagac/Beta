        ...
        // create load data dialog
        {
            ...
            for (int i = 1; i <= player.slots(); i++) {
                ...
                if (s == null) {
                    ...
                    info.add(new Label("Crafting Completed: " + s.progress, skin, "smaller")).expand().colspan(1)
                            .right().row();
    ...
    private static enum UIState implements State<NewUI> {
        Choose() {
            ...
            public void enter(final NewUI entity) {
                entity.slotFrame.addAction(Actions.sequence(Actions.alpha(0f), Actions.delay(1.5f),
                        Actions.alpha(1f, .3f), Actions.run(new Runnable() {
                            @Override
                            public void run() {
            ...
            public boolean onMessage(final NewUI entity, Telegram telegram) {
                ...
                if (index == 0) {
                    ...
                    }), Actions.alpha(0f, .4f), Actions.run(new Runnable() {
        ...
        Create() {
            ...
            @Override
            public void enter(final NewUI entity) {
                entity.createFrame.addAction(Actions.sequence(Actions.alpha(0f), Actions.delay(1.5f),
                        Actions.alpha(1f, .3f), Actions.run(new Runnable() {
                            @Override
                            public void run() {
        ...
        Story() {
            ...
            public void enter(final NewUI entity) {
                ...
                // goddess
                entity.goddess.addAction(Actions.sequence(Actions.alpha(0f), Actions.delay(4f), Actions.alpha(1f, 3f),
                        ...
                        }), Actions.forever(Actions.sequence(Actions.moveTo(entity.getWidth() * .6f, 58f, 5f),
            ...
            public void update(final NewUI entity) {
                if (story.hasNext()) {
                    ...
                    entity.text.addAction(Actions.sequence(Actions.alpha(0f, .1f), Actions.run(new Runnable() {
                    ...
                    entity.goddess.addAction(Actions.sequence(
                            ...
                            Actions.parallel(Actions.repeat(10, Actions.rotateBy(360f, .25f)),
                                    Actions.sequence(Actions.delay(1f), Actions.run(new Runnable() {

                                        @Override
                                        public void run() {
                                            entity.audio.playSfx(DataDirs.Sounds.shimmer);
                                        }

                                    }), Actions.moveTo(entity.goddess.getX(), entity.getHeight() + 128f, .4f)))));
                    entity.you.addAction(Actions.sequence(Actions.delay(5f),
                            Actions.moveTo(entity.getWidth() / 2f - entity.you.getWidth() / 2f, 48f, 2f),
                            Actions.delay(2f), Actions.alpha(0f, 2f), Actions.run(new Runnable() {
